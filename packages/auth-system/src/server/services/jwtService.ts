...
import { Pool } from 'pg';
import { JWTPayload, TokenPair, RefreshTokenData } from '../types';
...
export class JWTService {
  ...
  async generateRefreshToken(
    ...
    const refreshToken = jwt.sign(tokenPayload as any, JWT_REFRESH_SECRET, {
  ...
  async generateTokenPair(
    user: Omit<JWTPayload, 'iat' | 'exp' | 'aud' | 'iss'>,
    ipAddress?: string,
    userAgent?: string,
    deviceInfo?: Record<string, any>
  ): Promise<TokenPair> {
...
export function createJWTService(pool: Pool): JWTService {
  return new JWTService(pool);
}